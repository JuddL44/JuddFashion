...
export class Shop implements OnInit {
  ...
  applyFilter(tab: string, type: string)
  {
    ...
    if (tab === "All") { this.apiExtensionCategory = ''; }

    this.apiExtension = this.apiExtensionSearch + this.apiExtensionCategory + this.apiExtensionSortBy;
    this.currentProductDisplay = `${this.currentProductCategory}`

    if (this.currentProductCategory === "All") {this.currentProductDisplay += ' Products';}
    if (this.currentProductSearch !== "") { this.currentProductDisplay += ` - \"${this.currentProductSearch}\"`; }
    this.loadMoreProducts();
  }
}
